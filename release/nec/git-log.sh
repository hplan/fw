...
if [ $(stat -c %s /tmp/logBat.html ) -eq 0 ]; then
        ...
        sendemail -f hz_no_reply@grandstream.cn -t $MAIL_LIST -s smtp.grandstream.cn -o tls=no message-charset=utf-8 -xu hz_no_reply@grandstream.cn -xp S1pTestH2 -v -u "GXV3370 OEM NEC ${VERSION} git log" < /tmp/logBat2.html
fi
