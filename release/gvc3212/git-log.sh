...
export PROJ_TOP=/media/gshz/4T_Disk/gvc3212
...
if [[ $(stat -c %s ${Log_Raw}) -eq 0 ]]; then
        ...
        sendemail -f hz_no_reply@grandstream.cn -t ${MAIL_TO} -s smtp.grandstream.cn -o tls=no message-charset=utf-8 -xu hz_no_reply@grandstream.cn -xp S1pTestH2 -v -u "GVC3212 ${VERSION} git log" < ${Log_Pretty}
fi
