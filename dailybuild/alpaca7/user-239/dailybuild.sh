...
#            sendemail -f hz_no_reply@grandstream.cn -t $1 -s smtp.grandstream.cn -o tls=no message-charset=utf-8 -xu hz_no_reply@grandstream.cn -xp S1pTestH2 -v -u "GXV3380 user ${version} git log" < ${Log_Pretty}
            sendemail -f hz_no_reply@grandstream.cn -t $1 -s smtp.grandstream.cn -o tls=no message-charset=utf-8 -xu hz_no_reply@grandstream.cn -xp S1pTestH2 -v -u "GXV3380 git log" < ${Log_Pretty}
        fi
        return 0
    fi
}
...
build() {
    ...
    if ${BUILD_KERNEL}; then
        ...
        ## check again
        #if [[ ! -e ${PROJ_PATH}/android/out/target/product/cht_alpaca/boot.img ]]; then
        #    sendemail -f hz_no_reply@grandstream.cn -t $1 -s smtp.grandstream.cn -o tls=no message-charset=utf-8 -xu hz_no_reply@grandstream.cn -xp S1pTestH2 -v -u "GXV3380 user build kernel failed."
        #    exit 1
        #fi
    fi
